...
class GAN(object):
    def __init__(self, x_dim, noise_dim=100, g_arch=[128, 128],
    ...
    def define_dloss(self):
        return -tf.reduce_mean(tf.log(self.y_from_x) +
                              tf.log(1 - self.y_from_z))
        ...
        for epoch in range(epochs):
            ...
            sess.run(self.g_train_tf,
                {self.z_tf: z_noise,
                 self.lr_tf: g_lr,
                 self.keep_prob: 1.})
            g_tr_loss = sess.run(self.g_loss_tf,
                {self.z_tf: z_noise,
                 self.keep_prob: 1.})
            if g_tr_loss < 1e-5:
                break
            ...
            if nn_verbose:
                sys.stdout.write(('\rTraining epoch {}, time {}s. '
                    ...
                    'Generator loss {:.4g}.').format(
                        epoch, int(tr_time), d_tr_loss, g_tr_loss))
                sys.stdout.flush()
...
if __name__=="__main__":
    ...
    with tf.Session() as sess:
        ...
        X_samples = gan.sample(n_samples, nmin=X.min(), nmax=X.max())
